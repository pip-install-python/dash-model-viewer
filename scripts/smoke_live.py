...
def fetch(
    ...
    # Adding a keyword here is the 1.6.29 wake() hazard again: this repo's own
    # tests/test_smoke_live.py stubs `fetch`, and those stubs take `method` in
    # the same commit.
...
def main(base: str) -> int:
    ...
    # HTTP requires HEAD wherever GET is served. Werkzeug derives it from every
    # GET rule; FastAPI's APIRoute does not, so an ASGI-lane fork answers 405 to
    # the method most uptime monitoring probes with. One request, against the
    # route whose 405 reads as "the site is down" while the site is healthy.
