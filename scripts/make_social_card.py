...
ACCENT = (76, 110, 245)      # #4C6EF5 — manifest theme_color
TEXT = (245, 246, 247)
MUTED = (150, 158, 168)
...
def main() -> int:
    from lib.constants import BASE_URL, SITE_BRAND, SITE_DESCRIPTION

    default_domain = BASE_URL.split("://", 1)[-1].rstrip("/")

    ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    ap.add_argument("--artwork", default="github_assets/card-artwork.png",
    ...
    # Derived from SITE_BRAND, like --brand above, rather than hard-coded:
    # the boilerplate shipped its own tagline as the default here, so every
    # fork that ran this script with no arguments produced a correctly-branded
    # headline over the BOILERPLATE's description. Splitting the one constant
    # means the two halves of the card cannot disagree with each other or with
    # the site.
    ap.add_argument("--tagline",
                    default=(SITE_BRAND.split(" — ", 1)[1]
                             if " — " in SITE_BRAND else SITE_DESCRIPTION))
