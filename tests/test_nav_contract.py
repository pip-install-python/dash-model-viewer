...
def test_api_page_is_registered_because_this_fork_documents_a_package(app_module):
    """The template asserts the inverse (`API_PACKAGES == []`, no /api). This
    site documents `dash_model_viewer`, so the contract's other branch is the
    one that holds here: the page registers, the sidebar shows the API
    section, and /api/llms.txt carries the same tables as Markdown."""
    import dash

    from lib.constants import API_PACKAGES

    assert API_PACKAGES == ["dash_model_viewer"]
    assert "/api" in [p["path"] for p in dash.page_registry.values()]


def test_the_api_page_reads_this_package_without_a_metadata_json(app_module):
    """The 1.0.0 rebuild removed the generator, so this package ships no
    `metadata.json` (tests/test_no_regeneration.py fails if one returns).
    lib/api_reference falls back to the class docstring + __init__ signature —
    the same four columns from the sources this package has."""
    from pathlib import Path
    ...
    assert not (Path(dash_model_viewer.__file__).parent / "metadata.json").exists()
    comps = api_reference.load_package("dash_model_viewer")
    assert [c["name"] for c in comps] == ["ModelViewer", "Slot"]
    props = {p["name"]: p for p in comps[0]["props"]}
    assert props["src"]["required"] and props["alt"]["required"]
    assert not props["ar"]["required"] and props["ar"]["default"] == "True"
    assert props["alt"]["description"].startswith("Accessible description")
    assert comps[0]["props"][0]["name"] == "id", "id sorts first, like the generated tables"
    md = api_reference.as_markdown(["dash_model_viewer"])
    assert "| `src` * |" in md
    # Every pipe inside a cell is escaped, or a union type splits the row.
