...
# Fork points. Any two REAL pages on this site: the suite registers tiers on
# them directly, so they only need to exist in the page registry.
GATED_PAGE = "/api-reference"
PUBLIC_PAGE = "/camera-and-views"
...
def test_run_py_pins_the_funnel_public(app_module):
    ...
    # /quick-start is this site's funnel, the way /getting-started is the
    # template's: it is the page that teaches someone to install the package,
    # so gating it would gate the only path to becoming a user.
    for path in ("/", "/quick-start", "/llms-small.txt", "/llms-full.txt"):
